...
import { fromString } from "../shapez/src/js/game/item.js";
import * as op from "../shapez/src/js/game/operations.js";
import { Canvas } from "canvas";
export default new Command({
    name: "shapeparse",
    alias: ["parse"],
    desc: `Performs operations on shapez.io items.`,
    usage: `<figure it out yourself>`,
    execute: async (message, args) => {
        await message.channel.send(parse(args));
    },
});
class ItemStack extends Array {
    execute(inputs, outputs, operator) {
...
function parse(args) {
    const operations = args.map(arg => operationMap.get(arg) ?? [0, 1, () => [fromString(arg)]]);
...
//# sourceMappingURL=shapeparse.js.map